...
- **166 figures confirmed by two independent readings.** The same printed number was read from the PDF's text layer and again by OCR, with no disagreements. Any table extractor can be scored on whether it finds these values in the right cell.
...
**Measured, not assumed.** Before release, the table layer is scored against the 166 figures confirmed in 2026 and the values in `budget-history.csv`. How many does it contain, in the right cell and year, and how many of those carry grade A? These rates are published with the release, since breadth without a measured error rate would be a liability.
...
- **Score the layer** against the 166 twice-confirmed figures and `budget-history.csv`: recall, grade distribution, and the most common failure types. Publish the scores with the data.
