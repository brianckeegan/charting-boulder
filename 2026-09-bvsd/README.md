...
Every price and income figure is county grain. The Boulder MSA is Boulder County and contains no other county, so the two labels describe one territory; the peer basket has no metropolitan series that matches it, and every peer county has both a price and an income series. `similar-boulder.json` gives Iowa City the county code 19087 while naming Johnson County; 19087 is Henry County and Johnson County is 19103. `places.csv` carries the corrected code. The published `2025-06-population/similar-boulder.json` still has the original, and no finding in this folder depended on it.
...
- Across fifteen editions of the comprehensive plan, preservation language outnumbers families-and-schools language 8 to 1. The ratio is 14 to 1 before 2001 and 6 to 1 from 2001 on: it narrows, which reverses what outlining this piece expected. The level, not the trend, is the durable finding.
