    ...
    <div ref={rootRef} style={{ background: C.paper, color: C.ink, fontFamily: FONT }} className="w-full">
      ...
      <div className="bbw mx-auto" style={{ maxWidth: 680, padding: "8px 16px 56px" }}>
        ...
        <header className="pt-6 pb-5" style={{ borderBottom: `3px solid ${C.lime}` }}>
          ...
          <h1 style={{ fontSize: "clamp(25px,6.4vw,38px)", fontWeight: 800, lineHeight: 1.06, marginTop: 8, letterSpacing: "-0.02em" }}>Most of Boulder’s $521&nbsp;million is already spoken for.</h1>
          <p style={{ fontSize: 16, lineHeight: 1.5, color: C.inkSoft, marginTop: 12 }}>
            The city must pass a balanced budget every year. In 2026 it had a <strong style={{ color: C.ink }}>{fmt(SCENARIOS.y2026.gap)} General Fund shortfall</strong> to close as sales-tax revenue flattened. Move money, raise revenue, close the gap. The catch is what you’re allowed to touch — and what Colorado law won’t let you.
          </p>
        </header>
        ...
        <section className="mt-7">
          ...
          <p style={{ fontSize: 13.5, color: C.inkSoft, marginTop: 4 }}><strong style={{ color: C.ink }}>56%</strong> of the city’s 3.86% sales tax is already pledged to specific purposes. The four largest of those dedications are shown below; together with smaller ones they make up that 56%. These rates were set at the ballot box — the council can’t slide them, can’t cut them to plug the General Fund, and can’t move them to a new priority. The yearly revenue shown for each is approximate.</p>
        ...
        <section className="mt-7">
          ...
          <p style={{ fontSize: 13.5, color: C.inkSoft, marginTop: 4 }}>This includes the {fmt(113.3)} capital budget and every dedicated or fee-funded operating fund — down to small outlays like arts and eviction prevention. Adjust them all you like: the money stays inside each fund’s purpose and can’t fill the General Fund gap. That ring-fence is the whole point.</p>
          {showLocked && (
            <div className="mt-3 grid gap-2">
              {LOCKED_FUNDS.map((f) => {
        ...
        <section className="mt-7 rounded-lg p-4" style={{ background: C.wash, border: `1px solid ${C.hair}` }}>
            ...
            <div className="mt-4 rounded-lg p-3" style={{ background: C.limeTint, border: `2px solid ${demoCount > 0 ? C.limeDk : C.ink}` }}>
              <div className="flex items-start justify-between gap-3">
                <div style={{ minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 13, color: C.inkSoft, marginTop: 3 }}>Answer at least one question — even <em>“Prefer not to say.”</em> Everything else is optional, and it lets us compare how different parts of Boulder balanced it. Your answers and your budget stay on Boulder Reporting Lab’s own servers — never a third party’s.</div>
                </div>
          ...
          <div className="flex flex-wrap items-center gap-2 mt-4">
            <button onClick={submit} disabled={!balanced || submitted || demoCount === 0} className="flex items-center gap-1.5" style={{ fontSize: 13, fontWeight: 800, padding: "10px 18px", borderRadius: 7, cursor: balanced && !submitted && demoCount > 0 ? "pointer" : "not-allowed", border: `2px solid ${balanced && !submitted && demoCount > 0 ? C.ink : C.hair}`, background: balanced && !submitted && demoCount > 0 ? C.lime : C.hair, color: balanced && !submitted && demoCount > 0 ? C.ink : C.inkSoft }}><ArrowDownToLine size={14} /> {submitted ? "Added to the tally" : "Add my budget"}</button>
        ...
        <section className="mt-8" style={{ borderTop: `3px solid ${C.ink}`, paddingTop: 18 }}>
          <Eyebrow>What this can’t tell you</Eyebrow>
          <p style={{ fontSize: 15.5, lineHeight: 1.55, marginTop: 10 }}>A balanced ledger is not the same as a fair one. This tool counts dollars; it can’t weigh whether a fire inspector matters more than a rec-center subsidy, or who bears a sales tax that falls hardest on the people with the least. It also can’t move the walls between funds, or rewrite the rules that built them. Boulder’s budget isn’t hard to balance because the city overspends. It’s hard because the city long ago promised most of its money to specific purposes, one ballot measure at a time — and because TABOR leaves the council a narrow set of tools to begin with.</p>
          <p style={{ fontSize: 15.5, lineHeight: 1.55, marginTop: 12 }}>That is what the November 2026 <em>Fund Our Future</em> question is about. It asks less <em>how much</em> Boulder should raise than <em>how much of it the council should be free to move</em>. Whether that flexibility is worth giving up the certainty of dedicated taxes is not a number. It’s a judgment, and it’s yours.</p>
          <a href="https://github.com/brianckeegan/charting-boulder" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 mt-5" style={{ fontSize: 13.5, fontWeight: 800, color: C.ink, textDecoration: "none", border: `2px solid ${C.ink}`, borderRadius: 7, padding: "9px 14px" }}><Github size={15} /> Verify me: data, assumptions and code on GitHub</a>
          <div className="mt-5">
