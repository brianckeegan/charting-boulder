...
- [x] **The 384 unmatched yearbook district-years** — done, and it found two mis-attributions on the way (D27). 98.3% of district-years now carry a code, and 1977–85 joins the archive for the first time. What is left is 92 non-BOCES rows where one printed name covers two districts (Garfield, East/West Yuma County) or the district merged away before 2000, plus the BOCES, which are not districts. Each is named in `DATA-DICTIONARY.md`.
...
- [x] **A closure event study that handles staggered timing** — done, in section 8, with a Callaway–Sant'Anna estimator: 18 cohorts against districts not yet treated, 1,000 draws resampling whole districts. The flat enrolment path survives (−0.2% at five years) and the buildings effect gets **stronger**, because section 7's fade to 10.5% by the fifth year was the comparison group, not the districts. On clean comparisons it is still 16.3% down and excludes zero (D29).
...
- [x] **A sixth lever: fill the buildings** — done, in section 3. BVSD already draws 765 elementary pupils from outside its boundary, against the 666 it would take to lift every school that can reach the bar to it, and that flow rose 11.2% while the roll fell 19.8%. Seventy-eight years at the rate the district has actually managed, so it is a lever rather than a plan.
- [ ] **Costing the combinations** — every lever is scored alone. A redraw plus recruitment reaches further than either, because one rearranges children and the other adds them, and nothing here says what that costs.
