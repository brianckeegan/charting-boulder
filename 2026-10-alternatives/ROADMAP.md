...
- [ ] **2005–2012 teacher FTE** — the remaining eight years of school-grain CDE staffing. Those ratio PDFs place every character as its own positioned run and their line coordinates do not group reliably, so fields merge and rows scramble. The Datalab path used for the yearbooks is the likely route; it would cost about $7 for the eight volumes. NCES covers the years at both grains meanwhile, so this buys a second source rather than filling a hole.
- [ ] **District-grain teacher FTE from CDE** — `district-teacher-fte-cde.csv` has 17 rows. Most of CDE's district staff reports are average-salary tables carrying no FTE column; a parser for that shape would also yield a salary series the archive does not have.
